...
function getInfoFromApi(endpoint, params){
  callApi(endpoint, params, (data) => {
    ...
    data.articles.forEach((article) => {
      ...
      const hash = generateHash(representation)
      const card = `
...
function removerFavorito(hash){
  if(!isFavorito(hash))return

  localStorage.removeItem(hash)
}

// function getFavirotosFromLocalStorage(){
//   const favList = []
//   for (let index = 0; index < localStorage.length; index++) {
//     favList[index] = localStorage.
